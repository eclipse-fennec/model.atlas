...
/**
 * Origin service-property names stamped on every OSGi service the client
 * publishes for a remotely fetched artifact (Phase 3 / Phase 5).
 * <p>
 * These are deliberately distinct from the {@code emf.*} namespace (see
 * {@code org.eclipse.fennec.emf.osgi.constants.EMFNamespaces}) — in particular
 * {@link #ATLAS_SCOPE} is unrelated to {@code emf.model.scope}. The origin
 * properties are always set; they are not configurable. Always reference these
...
public final class AtlasProperties {
	 ...
	 * e.g. {@code cocl}. Used by the read-only {@code ScopedEObjectsRegistry}
	 ...
	 * {@code ScopedEObjectsRegistry} services (Phase 4 / Phase 5).
	 */
