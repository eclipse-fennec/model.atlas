...
public class MockTestHelper {
    ...
    public static class MockScopeServiceCollector extends ScopeServiceCollector {
        ...
        /* 
         * (non-Javadoc)
         * @see org.eclipse.fennec.model.atlas.workflow.ScopeServiceCollector#getScopeServiceByScopeName(java.lang.String)
         */
        @Override
    ...
    public static class MockRegistryServiceCollector extends RegistryServiceCollector {
    	...
    	/* 
    	 * (non-Javadoc)
    	 * @see org.eclipse.fennec.model.atlas.workflow.RegistryServiceCollector#getRegistryServiceByRegistryName(java.lang.String)
    	 */
    	@Override
    	public RegistryService<?> getRegistryServiceByRegistryName(String registryName) {
    		if(TEST_REGISTRY_NAME.equals(registryName)) return mockRegistryService;
    		return null;
    	}
    }
    ...
    public static class MockRegistryService implements RegistryService<EObject> {
		...
		public ObjectMetadata getMetadataFromStage(String scope, String stage, String objectId) {
            ...
            if (!objectId.equals(TEST_OBJECT_ID) && 
            		!objectId.equals("sensor-object-456") && 
            		!objectId.equals(new String(Base64.getUrlEncoder().encode(TEST_PACKAGE_NSURI.getBytes()))) &&
            		!objectId.equals(new String(Base64.getUrlEncoder().encode("http://readonly.com/schema/1.0".getBytes())))) {
                return null;
            }
            ...
            metadata.setScope(scope);
            metadata.setUploadTime(Instant.now());
            if("readonly-stage".equals(stage) || objectId.equals(new String(Base64.getUrlEncoder().encode("http://readonly.com/schema/1.0".getBytes())))) metadata.setIsReadOnly(true);
            return metadata;
		}
		...
		public Promise<Boolean> deleteFromStage(String scope, String stage, String objectId) {
            ...
            return Promises.resolved(
                objectId.equals(TEST_OBJECT_ID) || objectId.equals("sensor-object-456") || objectId.equals(new String(Base64.getUrlEncoder().encode(TEST_PACKAGE_NSURI.getBytes())))
            );
		}
		...
		public ObjectMetadata transitionToStage(String scope, String objectId, String fromStage, String toStage) {
			if(!isTransitionAllowed(fromStage, toStage)) {
				throw new IllegalArgumentException(String.format("Transition not allowed from %s to %s", fromStage, toStage));
			}
		...
		public boolean isValidStage(String stageName) {
			return TEST_STAGE_DRAFT.equals(stageName) || TEST_STAGE_APPROVED.equals(stageName) || TEST_STAGE_RELEASE.equals(stageName);
		}
