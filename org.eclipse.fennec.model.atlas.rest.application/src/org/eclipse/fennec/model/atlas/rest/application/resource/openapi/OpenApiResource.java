...
public class OpenApiResource extends BaseOpenApiResource {
    ...
    /**
     * Enhances the OpenAPI specification with supported media types for all
     * endpoints
     *
        ...
        oas.getPaths().forEach((pathName, pathItem) -> {
            pathItem.readOperations().forEach(operation -> {
                ...
                if (operation.getResponses() != null) {
                    operation.getResponses().forEach((statusCode, apiResponse) -> {
                        if (apiResponse != null && apiResponse.getContent() != null
                            ...
                            // Only add media types if content is already defined (meaning the response
                            // returns content)
                ...
                if (operation.getRequestBody() != null && operation.getRequestBody().getContent() != null
                    ...
                    // Only add media types if content is already defined (meaning the request
                    // expects a body)
                    // Add each supported media type if not already present
                    supportedMediaTypes.forEach(mediaType -> {
    ...
    private void enhanceWithScopeInformation(OpenAPI oas) {
        ...
        for (Scope scope : scopes) {
            ...
            if (scope.getRegistries() != null && !scope.getRegistries().isEmpty()) {
                scopeData.put("registries",
                        new ArrayList<>(scope.getRegistries().stream().map(r -> r.getName()).toList()));
            }
        ...
        scopeDescription.append(
                "A **Scope** is a \"tenant\" or logical partition within the Model Atlas registry. Scopes are hierarchical.\n\n");
        scopeDescription.append(
                "- `atlas`: A special, read-only \"system\" scope that contains common base models (like Ecore itself). ");
        ...
        scopeDescription
                .append("The hierarchy of scopes (defined by `parentScope`) governs visibility and uniqueness:\n\n");
        scopeDescription.append(
                "1. **Uniqueness (Write-Time):** A namespace URI must be unique within its \"visibility chain\". ");
        scopeDescription.append(
                "When creating a new package, the system checks if that namespace URI already exists in any stage of the scope ");
        scopeDescription.append(
                "OR in the released stage of any parent scope. If a conflict is found, the creation will be rejected.\n");
        scopeDescription.append(
                "2. **Visibility (Read-Time):** Any query for a package by its namespace URI will search this chain. ");
        scopeDescription.append(
                "The system will first look in the specified scope and stage. If not found, it will proceed to check the ");
        scopeDescription.append("released stage of the parent scope, and so on up to the `atlas` scope.\n\n");
        scopeDescription.append(
                "This ensures that released packages from parent scopes are resolvable from all child scopes.\n\n");
        ...
        for (Scope scope : scopes) {
            ...
            if (scope.getRegistries() != null && !scope.getRegistries().isEmpty()) {
                scopeDescription.append("  - Registries: ")
                        .append(String.join(", ", scope.getRegistries().stream().map(r -> r.getName()).toList()))
                        .append("\n");
            }
        }
