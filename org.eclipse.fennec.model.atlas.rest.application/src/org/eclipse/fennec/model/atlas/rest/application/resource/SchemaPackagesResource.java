...
public class SchemaPackagesResource {
	...
	public Response listPackagesInStage(
		...
		try {
			if (nsUri != null) {
				...
				ObjectMetadata metadata = scopeService.getMetadataFromStageForRegistry(REGISTRY_NAME, stageName, encodedUri);
				if (metadata == null) {
				...
				List<ObjectMetadata> objectsMetadata = scopeService.listInStageForRegistryByName(REGISTRY_NAME, stageName, name);
	...
	public Response getPackageContent(
		...
		try {
			...
			EPackage ePackage = (EPackage) scopeService.getContentFromStageForRegistry(REGISTRY_NAME, stageName, encodedNsUri);
			if (ePackage == null) {
				return Response.status(Response.Status.NO_CONTENT).build();
			}
	...
	public Response updatePackageContent(
		...
		try {
			...
			if (existingMetadata.isIsReadOnly()) {
				return Response.status(Response.Status.FORBIDDEN).entity(String.format("Schema %s is in read-only state",  nsUri)).build();
			}
	...
	public Response deletePackage(
		...
		try {
			...
			boolean deleted = scopeService.deleteFromStageForRegistry(REGISTRY_NAME, stageName, encodedNsUri).getValue();
			if (deleted) return Response.status(Response.Status.OK).build();
			return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(String.format("Schema %s deletion failed but causes are unknown", nsUri)).build();
