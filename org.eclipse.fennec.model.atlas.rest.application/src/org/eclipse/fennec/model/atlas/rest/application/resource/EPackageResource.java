...
/**
 * REST API for CRUD operations on EPackages. Provides endpoints to create,
 * read, update, and delete EMF EPackages at runtime.
 *
...
public class EPackageResource {
    ...
    @Consumes
    @Produces
    @Operation(summary = "Create a new EPackage", description = "Register a new EPackage in the system", responses = {
            @ApiResponse(responseCode = "201", description = "EPackage created successfully", content = @Content(schema = @Schema(implementation = EPackage.class))),
    ...
    public Response createEPackage(
            @Parameter(description = "The EPackage to register", required = true) EPackage ePackage) {
        ...
        try {
            ...
            return Response.status(Status.CREATED).header("Location", "/epackages?nsUri=" + ePackage.getNsURI())
                    .entity(registered.get()).type(mediaType).build();
    ...
    @GET
    @Produces
    @Operation(summary = "Get an EPackage or list all EPackages", description = "Get a specific EPackage by nsUri, or list all if no nsUri provided", responses = {
            @ApiResponse(responseCode = "200", description = "EPackage found or list returned", content = @Content(schema = @Schema(implementation = EPackage.class))),
    ...
    public Response getEPackage(
            @Parameter(description = "The namespace URI of the EPackage to retrieve") @QueryParam("nsUri") String nsUri,
            @Parameter(description = "Number of items to skip for pagination") @QueryParam("skip") @DefaultValue("0") int skip,
            @Parameter(description = "Maximum number of items to return") @QueryParam("limit") @DefaultValue("100") int limit) {
    ...
    @Consumes
    @Produces
    @Operation(summary = "Update an existing EPackage", description = "Update an EPackage identified by nsUri", responses = {
            @ApiResponse(responseCode = "200", description = "EPackage updated successfully", content = @Content(schema = @Schema(implementation = EPackage.class))),
    ...
    public Response updateEPackage(
            @Parameter(description = "The namespace URI of the EPackage to update", required = true) @QueryParam("nsUri") String nsUri,
            @Parameter(description = "The updated EPackage data", required = true) EPackage ePackage) {

        checkContentType();

        if (nsUri == null || nsUri.isEmpty()) {
            return Response.status(Status.BAD_REQUEST).entity("nsUri query parameter is required")
                    .type(MediaType.TEXT_PLAIN).build();
        }
    ...
    @DELETE
    @Operation(summary = "Delete an EPackage", description = "Unregister an EPackage identified by nsUri", responses = {
            @ApiResponse(responseCode = "204", description = "EPackage deleted successfully (or didn't exist)") })
    public Response deleteEPackage(
            @Parameter(description = "The namespace URI of the EPackage to delete", required = true) @QueryParam("nsUri") String nsUri) {
    ...
    private EPackageListResponse createListResponse(List<EPackage> packages, int skip, int limit) {
        ...
        packages.stream().skip(skip).limit(limit).forEach(pkg -> {
