 ...
 * 
 * <p>
 * This class automatically registers a mock PostReleaseActionService to satisfy
 * the mandatory dependency in EObjectWorkflowService.
 * </p>
 * 
...
public abstract class WorkflowTestBase {
    ...
    void setUpMockServices() {
        ...
        when(mockPostReleaseActionService.executePostUnreleaseActions(anyString(), anyString(), anyString(),
                anyString(), anyString(), anyString(), anyString())).thenReturn(promiseFactory.resolved(null));
        when(mockPostReleaseActionService.supportsObjectType(anyString())).thenReturn(true);
        when(mockPostReleaseActionService.getLastActionInfo(anyString())).thenReturn(null);
        ...
        postReleaseServiceRegistration = bundleContext.registerService(PostReleaseActionService.class,
                mockPostReleaseActionService, properties);
    }
    ...
    /**
     * Subclasses must provide access to the BundleContext. This is typically
     * injected by the OSGi test framework.
     * 
    ...
    /**
     * Helper method to verify that post-release actions were called. Subclasses can
     * use this for verification in their tests.
     * 
