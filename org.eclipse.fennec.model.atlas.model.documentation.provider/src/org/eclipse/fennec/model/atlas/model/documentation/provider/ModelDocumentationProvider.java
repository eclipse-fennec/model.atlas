 ...
 */
package org.eclipse.fennec.model.atlas.model.documentation.provider;
...
public class ModelDocumentationProvider {
    ...
    public Map<String, String> generateAllClassesDocumentation(EPackage ePackage) {
        ...
        List<EClass> eClasses = ePackage.getEClassifiers().stream().filter(c -> c instanceof EClass)
    ...
    public OutputStream generateDocumentation(EPackage ePackage, EcoreToDocumentationOptions option) {
        ...
        try {
            ...
            case HTML_WITH_PLANTUML_CLASS_DIAGRAM:
            case ONLY_HTML_CLASS_OVERVIEW:
            ...
            case ONLY_MARKDOWN_CLASS_OVERVIEW:
    ...
    public OutputStream retrieveDocumentation(String docFilePath, boolean generateIfNotFound, EPackage ePackage,
            EcoreToDocumentationOptions docOption) {
    ...
    public OutputStream retrieveDocumentation(String docFilePath, boolean generateIfNotFound, EClass eClass,
            EcoreToDocumentationOptions docOption) {
        OutputStream os = retrieveDocumentationFile(docFilePath);
        if (os == null && generateIfNotFound) {
    ...
    private Map<String, String> createDocumentationFileMap(EClass eClass) {
        ...
        documentationFileMap.put(ModelDocumentationConstants.PROPERTY_MD_FILE.concat(".").concat(eClass.getName()),
                Paths.get(config.output_root_folder(), config.output_md_folder(), eClass.getEPackage().getName()
                        .concat("_").concat(eClass.getName()).concat(ModelDocumentationConstants.MD_FILE_EXTENSION))
                        .toString());

        documentationFileMap.put(
                ModelDocumentationConstants.PROPERTY_MD_MERMAID_FILE.concat(".").concat(eClass.getName()),
                Paths.get(config.output_root_folder(), config.output_md_mermaid_folder(), eClass.getEPackage().getName()
                        .concat("_").concat(eClass.getName()).concat(ModelDocumentationConstants.MD_FILE_EXTENSION))
                        .toString());

        documentationFileMap.put(
                ModelDocumentationConstants.PROPERTY_MD_PLANTUML_FILE.concat(".").concat(eClass.getName()),
                Paths.get(config.output_root_folder(), config.output_md_plantuml_folder(),
                        eClass.getEPackage().getName().concat("_").concat(eClass.getName())
                                .concat(ModelDocumentationConstants.MD_FILE_EXTENSION))
                        .toString());

        documentationFileMap.put(ModelDocumentationConstants.PROPERTY_HTML_FILE.concat(".").concat(eClass.getName()),
                Paths.get(config.output_root_folder(), config.output_html_folder(), eClass.getEPackage().getName()
                        .concat("_").concat(eClass.getName()).concat(ModelDocumentationConstants.HTML_FILE_EXTENSION))
                        .toString());

        documentationFileMap.put(
                ModelDocumentationConstants.PROPERTY_HTML_MERMAID_FILE.concat(".").concat(eClass.getName()),
                Paths.get(config.output_root_folder(), config.output_html_mermaid_folder(),
                        eClass.getEPackage().getName().concat("_").concat(eClass.getName())
                                .concat(ModelDocumentationConstants.HTML_FILE_EXTENSION))
                        .toString());
        return documentationFileMap;
    }
}
