...
public class ApicurioStorageHelper extends AbstractStorageHelper {
     ...
     * @throws IOException
     */
    public ApicurioStorageHelper(ResourceSet resourceSet, EObjectRegistryService<EObject> registry,
	...
	if (isLatestVersionValid(latestVersion, objectId)) {
	    ...
	    URI objectUri = URI.createURI(apicurioURL.concat("/").concat(objectPath).concat("/versions/")
	...
	return doDelete(apicurioURL.concat("/").concat(objectId))
		&& doDelete(apicurioURL.concat("/").concat(objectId).concat(METADATA_EXTENSION));
    }
	...
	try {
	    SearchedArtifact searchResponse = (SearchedArtifact) sendGETRequest(
		    URI.createURI(apicurioURL.concat("/").concat(path)), MgmtApicurioPackage.Literals.SEARCHED_ARTIFACT,
		    "application/json");
	...
	URI searchVersionsURI = URI
		.createURI(config.base_url().concat("search/versions?orderBy=version&order=desc&artifactId=")
			.concat(artifactId).concat("&groupId=").concat(apicurioGroupId));
    ...
    private void postArtifact(String artifactId, EObject content, String contentType, ObjectMetadata metadata,
	...
	searchResponse.getArtifacts().forEach(a -> {
		...
		LOGGER.log(Level.SEVERE,
			String.format("IOException while updating registry cache at apicurio storage startup"), e);
	    }
	});
