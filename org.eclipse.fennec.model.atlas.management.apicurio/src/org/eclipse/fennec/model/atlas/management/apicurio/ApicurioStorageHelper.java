...
public class ApicurioStorageHelper extends AbstractStorageHelper {
     ...
     * @throws IOException
     */
    public ApicurioStorageHelper(ResourceSet resourceSet, EObjectRegistryService<EObject> objectRegistryService,
            EObjectApicurioStorageService.Config config) throws IOException {
        super(resourceSet);
     ...
     * 
     * @see
     * org.eclipse.fennec.model.atlas.mgmt.storage.AbstractStorageHelper#loadEObject
    ...
    public boolean deleteObject(String scope, String registry, String stage, String objectId) {
        ...
        return doDelete(constructApicurioURL(config.base_url(), groupId).concat("/").concat(objectId))
                && doDelete(constructApicurioURL(config.base_url(), groupId).concat("/").concat(objectId)
                        .concat(METADATA_EXTENSION));
    }
    ...
    protected String findObjectPath(String scope, String registry, String stage, String objectId) throws IOException {
        String groupId = createGroupId(scope, registry, stage);
        String url = config.base_url().concat("search/artifacts?groupId=").concat(groupId).concat("&artifactId=")
                .concat(objectId);
    ...
    protected boolean storageExists(String scope, String registry, String stage, String path) throws IOException {
        try {
            ...
            SearchedArtifact searchResponse = (SearchedArtifact) sendGETRequest(
                    URI.createURI(constructApicurioURL(config.base_url(), groupId).concat("/").concat(path)),
                    MgmtApicurioPackage.Literals.SEARCHED_ARTIFACT, "application/json");
    ...
    protected List<ObjectMetadata> loadAllStoredMetadata() throws IOException {
        ...
        // 2. List all artifacts for every group (remember to put the limit higher than
        // the default 20)
        // 3. If artifact id ends with METADATA_EXTENSION, then we load the metadata
        // object and the scope, registry, stage we find from the group Id
        // 4. Update registry cache
        List<ObjectMetadata> storedMetadata = new LinkedList<>();
        SearchGroupResponse groupResponse = (SearchGroupResponse) sendGETRequest(
                URI.createURI(config.base_url().concat("groups?limit=1000")),
                MgmtApicurioPackage.Literals.SEARCH_GROUP_RESPONSE, "application/json");
        for (Group group : groupResponse.getGroups()) {
            String[] groupIdSplit = group.getGroupId().split("-");
            if (groupIdSplit.length != 3) {
                throw new IllegalStateException(String
                        .format("groupId in apicurio not in the form scope-registry-stage: %s", group.getGroupId()));
            }
            ...
            SearchResponse artifactResponse = (SearchResponse) sendGETRequest(uri,
                    MgmtApicurioPackage.Literals.SEARCH_RESPONSE, "application/json");
            for (SearchedArtifact artifact : artifactResponse.getArtifacts()) {
                if (artifact.getArtifactId().endsWith(METADATA_EXTENSION)) {
                    ObjectMetadata metadata = loadMetadata(scope, registry, stage,
                            artifact.getArtifactId().replaceFirst(METADATA_EXTENSION, ""));
                    if (metadata != null)
                        storedMetadata.add(metadata);
                    else
                        LOGGER.warning(String.format(
                                "Loaded null metadata from apicurio for artifact id %s that was supposed to be in group %s",
                                artifact.getArtifactId().replaceFirst(METADATA_EXTENSION, ""),
                                scope.concat("-" + registry).concat("-" + stage)));
                }
            }
        }
    ...
    private SearchedVersion getLatestVersionForArtifactId(String scope, String registry, String stage,
    ...
    private void postArtifact(String scope, String registry, String stage, String artifactId, EObject content,
            String contentType, ObjectMetadata metadata, boolean isMetadata) throws IOException {
        ...
        ResourceOperation objectOp = createResource(
                URI.createURI(constructApicurioURL(config.base_url(), groupId).concat("?ifExists=CREATE_VERSION")),
                "application/json");
