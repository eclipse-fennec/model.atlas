...
### F21 · major · osgi-ds · management ✅ FIXED (2026-07-24)
...
- **Status:** ✅ Fixed 2026-07-24 — the constructor now submits `initializeCache()` via the `promiseFactory` instead of running it inline, so DS activation never blocks on storage I/O; readers fall back to the lazy `ensureCacheInitialized()` path if initialization hasn't completed or failed. Fixing this required closing F90 in the same change (see below) — with lazy init, finders skipping `ensureCacheInitialized` would have returned empty on first access. Regression test `testConstructorDoesNotLoadSynchronously` added. Management unit tests 126/126, file + apicurio OSGi ITs green.

### F22 · major · osgi-ds · management / workflow / readable.scope.collector / data.atlas.jpa.rest ✅ FIXED (2026-07-24)
...
- **Status:** ✅ Fixed 2026-07-24 — all six sites converted to the two-arg `remove(key, service)`: `BasicStorageRegistry.removeStorageService`, `ScopeServiceCollector.unbindScopeService`, `RegistryServiceCollector.unbindRegistryService`, `ReadableScopeCollector.unbindScopeService`, and the three unbind methods in `JpaDataResourceFilter`. Rebind-order regression test `testRemoveStorageServiceAfterReplacement` added to `BasicStorageRegistryTest`. Management unit tests 127/127, workflow OSGi ITs green; the other two bundles compile clean (readable.scope.collector has no test bundle; the jpa.rest ITs were not run — they have a known pre-existing unrelated failure).
