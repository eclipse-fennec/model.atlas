...
/**
 * Integration tests for data integrity validation by corrupting actual stored
 * files.
 * 
 * <p>
 * This test class validates that the storage system properly detects and
 * handles data corruption scenarios by actually modifying stored metadata files
 * on disk and verifying that appropriate exceptions are thrown.
 * </p>
 * 
 * <h3>Test Scenarios</h3>
 * <ul>
 * <li><strong>ObjectId Mismatch</strong> - Metadata file contains different
 * objectId than filename</li>
 * <li><strong>Missing ObjectId</strong> - Metadata file has null or empty
 * objectId</li>
 * <li><strong>Corrupted File Content</strong> - Invalid XMI content that fails
 * to parse</li>
 * </ul>
 * 
...
public class DataIntegrityValidationIntegrationTest {
    ...
    /**
     * Tests that loading metadata with corrupted objectId throws appropriate
     * exception.
     * 
     * <p>
     * This test:
     ...
     * <li>Verifies that data integrity violation is detected and exception
     * thrown</li>
     * </ol>
     */
    ...
    public void testLoadMetadataWithCorruptedObjectIdThrowsException(
        ...
        assertTrue(storageService.exists(TEST_SCOPE, TEST_REGISTRY, TEST_STAGE, storedObjectId),
                "Object should exist after storage");
        ...
        String corruptedContent = originalContent.replace("objectId=\"" + objectId + "\"",
    ...
    /**
     * Tests that loading metadata with missing objectId throws appropriate
     * exception.
     * 
     * <p>
     * This test:
     ...
     * <ol>
     * <li>Stores a valid object with metadata</li>
     * <li>Manually corrupts the metadata file by removing the objectId
     * attribute</li>
     * <li>Attempts to retrieve the metadata</li>
     * <li>Verifies that data integrity violation is detected and exception
     * thrown</li>
    ...
    public void testLoadMetadataWithMissingObjectIdThrowsException(
        ...
        assertTrue(failure.getCause().getMessage().contains("Data integrity violation"),
                "Should mention data integrity violation");
        assertTrue(failure.getCause().getMessage().contains("has no objectId set"), "Should mention missing objectId");
    }

    /**
     * Tests that loading completely corrupted metadata file throws appropriate
     * exception.
     * 
     * <p>
     * This test:
