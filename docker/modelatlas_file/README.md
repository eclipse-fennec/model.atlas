...
| `MODEL_ATLAS_SCOPE` | `jena` | Name of the tenant scope this image defines. The GDPR review runs in it: it triggers the review history there and writes review status there. Must match `MODEL_ATLAS_SCOPE` on the fennec-gdpr MCP half. Choose it at first deployment: the file backend stores objects under `STORAGE_ROOT/<scope>/`, so renaming the scope of a running instance does not move its data — the new scope comes up empty and the old folder stays behind (the `Storage Scopes` health check reports that). The initial-models folder is seeded per scope (`scopes/<name>/`), so a renamed scope needs its folder renamed, or it is silently not seeded. Use a plain name or leave it unset: an empty value does not fall back to `jena`, it registers a nameless scope. Passed through by `docker-compose-file.yml`. See [GDPR review history](../../docs/gdpr-review-history.md#configuration). |
